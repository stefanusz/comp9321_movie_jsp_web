...
<c:choose>
	<c:when test='${not empty nowShowing}'>
		...
		<c:forEach var="data" items="${nowShowing}">
			<tr><td><img src="${data.poster}" alt="capture_test" height="128" width="74"><td><a href='control?viewDetail=1&movieid=${data.movieID}'>${data.title}</a><td>${data.ratingString}
		</c:forEach>
		</table>
	</c:when>
...
<c:choose>
	<c:when test='${not empty nowShowing}'>
		...
		<c:forEach var="data" items="${comingSoon}">
			<tr><td><img src="${data.poster}" alt="capture_test" height="128" width="74"><td><a href='control?viewDetail=1&movieid=${data.movieID}'>${data.title}</a><td>${data.ratingString}<td>${data.releaseDate}
		</c:forEach>
		</table>
	</c:when>
